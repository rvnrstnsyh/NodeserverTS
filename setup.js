...
for (let i = 0; i < managers.length; i++) {
  ...
  for (const key in devDeps) {
    ...
    if (iteration[0] === devDepsLength - 1)(0, fs_1.writeFileSync)(tmpFile, `${key}@latest ${managers[i] === 'npm' ? '--save-dev' : '--dev'} && \\\n\n\t\t${rs256} && \\\n\t\trm -rf ./updater.tmp.sh\n\t}\n}\n\n`, flag);
    iteration[0]++;
  }
...
(0, fs_1.readFile)(envFileExample, 'utf8', (error, data) => {
  ...
  for (let i = 0; i < lines.length - 1; i++) {
    ...
    if (alpha.test(lines[i].substring(0, 1))) {
      ...
      env[parts[0].trim()] = [`${i}+index+`].concat(parts[1].split('')).join('');
    }
  }
  ...
  for (const key in newEnv)env[key] = `${env[key].split('+index+')[0]}+index+${newEnv[key]}`;
  ...
  for (let i = 0; i < lines.length - 1; i++) {
    ...
    for (const key in env) {
      //
      if (parseInt(env[key].split('+index+')[0]) === i)(0, fs_1.writeFileSync)(envFile, `${key}=${env[key].split('+index+')[1]}\n`, flag);
    }
  }
});
